...
export const CHAIN_ICONS: Record<string, string> = {
  ...
  base: "/tokens/base.png", // Fallbacks if any
...
export type LivePrices = Record<string, {
  usd: number;
  usd_24h_change: number;
  image?: string;
}>;
...
export const TOKENS: TokenInfo[] = [
  {
    ...
    price: 130,
    decimals: 9,
    defaultBalance: 0.05178,
    color: '#9945FF',
    icon: '◎',
    logoUrl: '/tokens/sol.webp',
    coingeckoId: 'solana',
  },
  {
    symbol: 'USDT',
...
export const DEFAULT_BALANCES: TokenBalance[] = TOKENS.map((t) => ({
  symbol: t.symbol,
...
export function formatCurrency(value: number, currency: string = 'USD', decimals: number = 2): string {
  ...
  // For tiny token prices (meme coins), show more decimals so it doesn't just show $0.00
  if (value > 0 && value < 0.01) {
    minDecimals = 2; // Keep at least 2
    maxDecimals = 8; // Show up to 8
  }
...
export function formatBalance(value: number): string {
  if (value === 0) return '0';
  if (value >= 1) return value.toLocaleString('en-US', { minimumFractionDigits: 3, maximumFractionDigits: 3 });
