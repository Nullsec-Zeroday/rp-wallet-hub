...
import { checkAdminAuth, clearLicenseDevices } from "@/lib/admin-license-store";
...
export async function POST(request: Request) {
  ...
  try {
    const result = await clearLicenseDevices(licenseKey);
    if (!result) return NextResponse.json({ error: "License not found" }, { status: 404 });
    return NextResponse.json(result);
  } catch (error) {
    console.error("[admin-keys] Failed to clear devices", error);
