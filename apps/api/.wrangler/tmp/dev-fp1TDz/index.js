...
function listWalletApps(activatedWallets = []) {
  return Object.values(walletRegistry).map((wallet) => ({
    ...wallet,
    activated: activatedWallets.includes(wallet.id)
  }));
}
__name(listWalletApps, "listWalletApps");

// ../../packages/config/src/index.ts
var referralBonusDays = {
  starter: 2,
  popular: 7,
  yearly: 30
};
function getReferralBonusDays(planId) {
  return referralBonusDays[planId] || 0;
}
__name(getReferralBonusDays, "getReferralBonusDays");
...
var licenseStatus = pgEnum("license_status", ["active", "expired", "revoked"]);
var walletAppId = pgEnum("wallet_app_id", ["phantom", "trust"]);
var transactionType = pgEnum("transaction_type", [
  "send",
  "receive",
  "swap",
  "buy",
...
var supportTicketType = pgEnum("support_ticket_type", ["did_not_receive_key", "bug"]);
var supportTicketStatus = pgEnum("support_ticket_status", ["open", "in_progress", "resolved", "closed"]);
...
var paymentOrders = pgTable(
  "payment_orders",
  {
    ...
    affiliateCheckoutIntentId: text("affiliate_checkout_intent_id").references(() => affiliateCheckoutIntents.id),
    affiliateVisitorId: text("affiliate_visitor_id"),
    affiliateClickId: text("affiliate_click_id").references(() => affiliateClicks.id),
...
var demoSessions = pgTable("demo_sessions", {
  id: text("id").primaryKey(),
  demoKeyId: text("demo_key_id").notNull().references(() => demoKeys.id),
  demoDeviceId: text("demo_device_id").notNull().references(() => demoDevices.id),
  userId: text("user_id").notNull().references(() => users.id),
  licenseId: text("license_id").notNull().references(() => licenses.id),
...
var walletApps = pgTable("wallet_apps", {
...
// src/abandoned-checkout.ts
function selectAbandonedReminderOrders(candidates, recentPurchaseEmails, limit) {
  const recentBuyers = new Set([...recentPurchaseEmails].map(normalizeEmail));
  const selectedBuyers = /* @__PURE__ */ new Set();
  const selected = [];
...
__name(selectAbandonedReminderOrders, "selectAbandonedReminderOrders");
...
var DEFAULT_WALLET_USERNAME = "RPWallet";
var DEMO_KEY_LABEL = "Free Demo";
var DEMO_LICENSE_PLAN = "Demo Preview";
...
var DemoUnavailableError = class extends Error {
  static {
    __name(this, "DemoUnavailableError");
  }
...
var DemoDeviceUsedError = class extends Error {
  static {
    __name(this, "DemoDeviceUsedError");
  }
  constructor() {
...
__name(getPlatformStore, "getPlatformStore");
var InMemoryPlatformStore = class {
  static {
    __name(this, "InMemoryPlatformStore");
  }
  paymentOrders = /* @__PURE__ */ new Map();
  supportTickets = /* @__PURE__ */ new Map();
  users = /* @__PURE__ */ new Map();
  licenses = /* @__PURE__ */ new Map();
  sessions = /* @__PURE__ */ new Map();
  launchTokens = /* @__PURE__ */ new Map();
  devices = /* @__PURE__ */ new Map();
  demoKeys = /* @__PURE__ */ new Map();
  demoDevices = /* @__PURE__ */ new Map();
  demoSessions = /* @__PURE__ */ new Map();
  walletProfiles = /* @__PURE__ */ new Map();
  ...
  async updatePaymentOrderProvider(id, providerPaymentId, status) {
    const order = this.paymentOrders.get(id);
    if (!order) return;
    this.paymentOrders.set(id, { ...order, providerPaymentId, status, updatedAt: (/* @__PURE__ */ new Date()).toISOString() });
  }
  async updatePaymentOrderStatus(id, status) {
    const order = this.paymentOrders.get(id);
    if (!order) return;
    this.paymentOrders.set(id, { ...order, status, updatedAt: (/* @__PURE__ */ new Date()).toISOString() });
  }
  async getAbandonedPaymentOrders(query) {
    const now = Date.now();
    const recentCutoff = now - query.olderThanMinutes * 60 * 1e3;
    const oldCutoff = now - query.maxAgeHours * 60 * 60 * 1e3;
    const recentPurchaseCutoff = now - query.recentPurchaseHours * 60 * 60 * 1e3;
    const recentPurchaseEmails = [...this.licenses.values()].filter((license) => license.status === "active").map((license) => this.users.get(license.userId)).filter((user) => Boolean(user?.email && Date.parse(user.createdAt) >= recentPurchaseCutoff)).map((user) => user.email);
    const candidates = [...this.paymentOrders.values()].filter((order) => {
    ...
    }).sort((a2, b2) => Date.parse(a2.createdAt) - Date.parse(b2.createdAt));
    return selectAbandonedReminderOrders(candidates, recentPurchaseEmails, query.limit);
  }
  async markAbandonedRemindersSent(email) {
  ...
  async completePaymentOrder(id, providerPaymentId, licenseId) {
    const order = this.paymentOrders.get(id);
  ...
  async getAdminSupportTickets() {
    return [...this.supportTickets.values()].sort((a2, b2) => Date.parse(b2.createdAt) - Date.parse(a2.createdAt));
  }
  async updateAdminSupportTicket(id, input) {
    const ticket = this.supportTickets.get(id);
    if (!ticket) return null;
    const updated = {
      ...ticket,
      status: input.status || ticket.status,
      adminNotes: input.adminNotes === void 0 ? ticket.adminNotes : normalizeOptionalString(input.adminNotes),
  ...
  async getAffiliateAttributionByClickId(clickId) {
    ...
    if (!attribution || attribution.clickId !== click.id || attribution.affiliateId !== click.affiliateId || new Date(attribution.expiresAt) <= /* @__PURE__ */ new Date() || !affiliate || affiliate.status !== "active") {
      return null;
    }
    return {
  ...
  async getAffiliateAttributionByBuyerEmail(email) {
    const normalizedEmail = email.trim().toLowerCase();
    const cutoff = Date.now() - AFFILIATE_ATTRIBUTION_TTL_MS;
    const intent = [...this.affiliateCheckoutIntents.values()].filter((entry) => entry.buyerEmail?.trim().toLowerCase() === normalizedEmail && Date.parse(entry.createdAt) >= cutoff && entry.clickId).sort((left, right) => Date.parse(right.createdAt) - Date.parse(left.createdAt))[0];
  ...
  async createAffiliateCheckoutIntent(input) {
    ...
    if (!attribution || attribution.affiliateId !== affiliate.id || attribution.affiliateCode !== affiliate.code || attribution.clickId !== input.clickId || new Date(attribution.expiresAt) <= /* @__PURE__ */ new Date()) return { accepted: false };
    const intent = {
      id: createId("afi"),
      affiliateId: affiliate.id,
      affiliateCode: affiliate.code,
  ...
  async createAffiliateConversion(input) {
    ...
    const affiliate = exactMatch?.affiliate || (input.affiliateCode ? this.getAffiliateByCode(normalizeAffiliateCode(input.affiliateCode)) : void 0);
    if (!affiliate || affiliate.status !== "active") return { accepted: false };
    const match2 = exactMatch;
  ...
  async updateAffiliateConversionStatus(id, status) {
    ...
    const updated = { ...entry[1], status, updatedAt: (/* @__PURE__ */ new Date()).toISOString() };
  ...
  async activateDemo(input, config) {
    ...
    const now = /* @__PURE__ */ new Date();
    const expiresAt = new Date(now.getTime() + config.durationMinutes * 60 * 1e3);
  ...
  async getAdminUnusedActiveLicenses() {
    const now = /* @__PURE__ */ new Date();
    return [...this.licenses.values()].filter((license) => license.status === "active" && new Date(license.expiresAt) > now).filter((license) => [...this.devices.values()].every((device) => device.userId !== license.userId)).sort((a2, b2) => new Date(a2.expiresAt).getTime() - new Date(b2.expiresAt).getTime()).map((license) => {
  ...
  async createWalletLaunch(input) {
    const session = this.sessions.get(input.sessionId);
    const wallet = walletRegistry[input.walletAppId];
    if (!session || !wallet?.enabled || new Date(session.expiresAt) <= /* @__PURE__ */ new Date()) return null;
  ...
  async exchangeWalletBootstrap(input) {
    const tokenHash = await hashToken(input.token);
    const launchToken = this.launchTokens.get(tokenHash);
    if (!launchToken || launchToken.consumedAt || new Date(launchToken.expiresAt) <= /* @__PURE__ */ new Date()) return null;
  ...
  async getWalletState(sessionId, walletAppId2) {
    const session = this.sessions.get(sessionId);
    if (!session || new Date(session.expiresAt) <= /* @__PURE__ */ new Date()) return null;
    const user = this.users.get(session.userId);
    const license = this.licenses.get(session.licenseId);
  ...
  async deleteWalletAccount(sessionId, walletAppId2, accountId) {
    const session = this.sessions.get(sessionId);
    if (!session || new Date(session.expiresAt) <= /* @__PURE__ */ new Date()) return null;
    const user = this.users.get(session.userId);
    const license = this.licenses.get(session.licenseId);
  ...
  async getWalletTransactions(sessionId, walletAppId2) {
    ...
    if (!user) return null;
    const profile = this.getOrCreateWalletProfile(user.id, walletAppId2, walletRegistry[walletAppId2].name);
    return [...this.walletTransactions.get(profile.id) || []].sort((a2, b2) => Date.parse(b2.createdAt) - Date.parse(a2.createdAt));
  }
  async createWalletTransaction(sessionId, input) {
    const session = this.sessions.get(sessionId);
    if (!session || new Date(session.expiresAt) <= /* @__PURE__ */ new Date()) return null;
    const user = this.users.get(session.userId);
    const license = this.licenses.get(session.licenseId);
    ...
    const toAmount = input.toAmount ? Number(input.toAmount) : void 0;
    ...
    const transaction = {
      ...
      toTokenSymbol: effectiveType === "swap" ? toTokenSymbol : void 0,
      toAmount: effectiveType === "swap" && toAmount ? formatAmount(toAmount) : void 0,
  ...
  buildWalletBootstrap(user, license, walletAppId2) {
  ...
  createLicense(keyHash, userId, now) {
    const license = {
      id: createId("lic"),
      keyHash,
      userId,
  ...
  createSession(userId, licenseId, expiresAt) {
    const session = {
      id: createId("ses"),
      userId,
      licenseId,
  ...
  getActivatedWallets(userId) {
    return [...this.walletProfiles.values()].filter((profile) => profile.userId === userId).map((profile) => profile.walletAppId);
  }
  getOrCreateWalletProfile(userId, walletAppId2, displayName) {
    const id = `${userId}:${walletAppId2}`;
  ...
  getOrCreateWalletAccounts(profile) {
  ...
  buildAffiliateDashboard(affiliate, baseUrl) {
    ...
    const commission = /* @__PURE__ */ __name((status) => conversions.filter((conversion) => status ? conversion.status === status : conversion.status !== "rejected").reduce((total, conversion) => total + Number(conversion.commissionAmount), 0).toFixed(2), "commission");
    return {
      affiliate,
      ...
      recentClicks: clicks.sort((a2, b2) => Date.parse(b2.createdAt) - Date.parse(a2.createdAt)).slice(0, 10).map(toAffiliateDashboardClick),
      recentCheckoutIntents: checkoutIntents.sort((a2, b2) => Date.parse(b2.createdAt) - Date.parse(a2.createdAt)).slice(0, 10).map(toAffiliateDashboardIntent),
      recentConversions: conversions.sort((a2, b2) => Date.parse(b2.createdAt) - Date.parse(a2.createdAt)).slice(0, 10).map(toAffiliateDashboardConversion)
    };
  }
  findAffiliateForConversion(input) {
    if (input.checkoutIntentId) {
      ...
      if (input.affiliateCode && intent.affiliateCode !== normalizeAffiliateCode(input.affiliateCode)) return void 0;
      const affiliate = this.affiliates.get(intent.affiliateId);
...
var NeonPlatformStore = class {
  ...
  async createPaymentOrder(input) {
    const [order] = await this.db.insert(paymentOrders).values({
      id: createId("pay"),
      provider: input.provider,
      email: input.email,
      planId: input.planId,
  ...
  async getPaymentOrder(id) {
    const [order] = await this.db.select().from(paymentOrders).where(eq(paymentOrders.id, id)).limit(1);
    return order ? toPaymentOrderSummary(order) : null;
  }
  async updatePaymentOrderProvider(id, providerPaymentId, status) {
    await this.db.update(paymentOrders).set({ providerPaymentId, status, updatedAt: /* @__PURE__ */ new Date() }).where(eq(paymentOrders.id, id));
  }
  async updatePaymentOrderStatus(id, status) {
    await this.db.update(paymentOrders).set({ status, updatedAt: /* @__PURE__ */ new Date() }).where(eq(paymentOrders.id, id));
  }
  async completePaymentOrder(id, providerPaymentId, licenseId) {
    const now = /* @__PURE__ */ new Date();
    const completed = await this.db.update(paymentOrders).set({ providerPaymentId, licenseId, status: "finished", updatedAt: now, fulfilledAt: now }).where(and(eq(paymentOrders.id, id), isNull(paymentOrders.licenseId))).returning({ id: paymentOrders.id });
    return completed.length > 0;
  }
  async getAbandonedPaymentOrders(query) {
    const now = Date.now();
    const recentCutoff = new Date(now - query.olderThanMinutes * 60 * 1e3);
    const oldCutoff = new Date(now - query.maxAgeHours * 60 * 60 * 1e3);
    const recentPurchaseCutoff = new Date(now - query.recentPurchaseHours * 60 * 60 * 1e3);
    const [orders, recentPurchases] = await Promise.all([
      this.db.select().from(paymentOrders).where(
        and(
          eq(paymentOrders.status, "waiting"),
          isNull(paymentOrders.licenseId),
          isNull(paymentOrders.abandonedReminderSentAt),
          lt(paymentOrders.createdAt, recentCutoff),
          gt3(paymentOrders.createdAt, oldCutoff)
        )
      ).orderBy(asc(paymentOrders.createdAt)),
      this.db.select({ email: users.email }).from(licenses).innerJoin(users, eq(licenses.userId, users.id)).where(and(eq(licenses.status, "active"), gt3(licenses.createdAt, recentPurchaseCutoff)))
    ]);
  ...
  async markAbandonedRemindersSent(email) {
    await this.db.update(paymentOrders).set({ abandonedReminderSentAt: /* @__PURE__ */ new Date() }).where(
      and(
        eq(paymentOrders.email, email.trim().toLowerCase()),
        eq(paymentOrders.status, "waiting"),
        isNull(paymentOrders.licenseId)
      )
    );
  ...
  async getAdminSupportTickets() {
    const tickets = await this.db.select().from(supportTickets).orderBy(desc(supportTickets.createdAt)).limit(500);
  ...
  async updateAdminSupportTicket(id, input) {
    ...
    if (input.adminNotes !== void 0) values.adminNotes = normalizeOptionalString(input.adminNotes);
    const [ticket] = await this.db.update(supportTickets).set(values).where(eq(supportTickets.id, id)).returning();
  ...
  async getAffiliateAttributionByClickId(clickId) {
    const [click] = await this.db.select().from(affiliateClicks).where(eq(affiliateClicks.id, clickId)).limit(1);
    if (!click) return null;
    const [attribution] = await this.db.select().from(affiliateAttributions).where(eq(affiliateAttributions.visitorId, click.visitorId)).limit(1);
    if (!attribution || attribution.clickId !== click.id || attribution.expiresAt <= /* @__PURE__ */ new Date()) return null;
  ...
  async getAffiliateAttributionByBuyerEmail(email) {
    ...
    const [intent] = await this.db.select().from(affiliateCheckoutIntents).where(and(eq(affiliateCheckoutIntents.buyerEmail, normalizedEmail), gt3(affiliateCheckoutIntents.createdAt, cutoff))).orderBy(desc(affiliateCheckoutIntents.createdAt)).limit(1);
    if (!intent?.clickId) return null;
  ...
  async createAffiliateCheckoutIntent(input) {
    ...
    if (!attribution || attribution.affiliateId !== affiliate.id || attribution.affiliateCode !== affiliate.code || attribution.clickId !== input.clickId || attribution.expiresAt <= /* @__PURE__ */ new Date()) return { accepted: false };
  ...
  async createAffiliateConversion(input) {
    ...
    if (input.checkoutIntentId && !match2) return { accepted: false };
    const affiliate = match2?.affiliate || (input.affiliateCode ? await this.getActiveAffiliateByCode(input.affiliateCode) : void 0);
    if (!affiliate) return { accepted: false };
    const amount = normalizeMoney(input.amount);
  ...
  async updateAffiliateConversionStatus(id, status) {
    const [existing] = await this.db.select().from(affiliateConversions).where(eq(affiliateConversions.id, id)).limit(1);
    if (!existing || !isAllowedAffiliateStatusTransition(existing.status, status)) return null;
    const [updated] = await this.db.update(affiliateConversions).set({ status, updatedAt: /* @__PURE__ */ new Date() }).where(eq(affiliateConversions.id, id)).returning();
    return updated ? toAffiliateConversionSummary(updated) : null;
  }
  async getAffiliateAdminSnapshot() {
    const [affiliates2, clicks, checkoutIntents, conversions, payoutRows] = await Promise.all([
      ...
      this.db.select({
        ...
        total: sum(affiliateConversions.commissionAmount)
      }).from(affiliateConversions).groupBy(affiliateConversions.affiliateId, affiliateConversions.status)
    ]);
  ...
  async activateDemo(input, config) {
    ...
    if (!input.deviceId?.trim()) throw new DemoUnavailableError("deviceId is required");
    await this.ensureWalletApps();
    const existingDevice = await this.db.select().from(demoDevices).where(eq(demoDevices.deviceId, input.deviceId)).limit(1);
    ...
    await this.recordDevice(user.id, input.deviceId);
    const [demoDevice] = await this.db.insert(demoDevices).values({
      id: createId("ddv"),
      deviceId: input.deviceId,
      firstSeenAt: now,
      lastSeenAt: now
  ...
  async getAdminUnusedActiveLicenses() {
    const rows = await this.db.select({
      id: licenses.id,
      keyPlaintext: licenses.keyPlaintext,
      userId: licenses.userId,
      email: users.email,
      plan: licenses.plan,
      expiresAt: licenses.expiresAt,
      status: licenses.status,
      allowedDevices: licenses.allowedDevices,
      createdAt: licenses.createdAt,
      updatedAt: licenses.updatedAt,
      deviceCount: count(devices.id)
    }).from(licenses).innerJoin(users, eq(users.id, licenses.userId)).leftJoin(devices, eq(devices.userId, licenses.userId)).where(and(eq(licenses.status, "active"), gt3(licenses.expiresAt, /* @__PURE__ */ new Date()))).groupBy(
      licenses.id,
      licenses.keyPlaintext,
      licenses.userId,
      users.email,
      licenses.plan,
      licenses.expiresAt,
      licenses.status,
      licenses.allowedDevices,
      licenses.createdAt,
      licenses.updatedAt
    ).having(({ deviceCount }) => eq(deviceCount, 0)).orderBy(asc(licenses.createdAt)).limit(250);
    return rows.map((row) => ({
      id: row.id,
      keyPlaintext: row.keyPlaintext ?? void 0,
      userId: row.userId,
  ...
  async createWalletLaunch(input) {
    ...
    await this.db.insert(walletLaunchTokens).values({
      id: createId("wlt"),
      tokenHash: await hashToken(rawToken),
      sessionId: session.id,
      walletAppId: input.walletAppId,
  ...
  async exchangeWalletBootstrap(input) {
    const tokenHash = await hashToken(input.token);
    const [launchToken] = await this.db.select().from(walletLaunchTokens).where(and(eq(walletLaunchTokens.tokenHash, tokenHash), isNull(walletLaunchTokens.consumedAt))).limit(1);
    if (!launchToken || launchToken.expiresAt <= /* @__PURE__ */ new Date()) return null;
  ...
  async deleteWalletAccount(sessionId, walletAppId2, accountId) {
    ...
    const account = accounts.find((entry) => entry.id === accountId);
    if (!account || accounts.length <= 1) return null;
    await this.db.delete(walletEvents).where(eq(walletEvents.accountId, accountId));
    await this.db.delete(walletNotifications).where(eq(walletNotifications.accountId, accountId));
    await this.db.delete(walletTransactions).where(eq(walletTransactions.accountId, accountId));
    await this.db.delete(walletBalances).where(eq(walletBalances.accountId, accountId));
    await this.db.delete(walletAccounts).where(eq(walletAccounts.id, accountId));
    return this.buildWalletBootstrap(user, license, walletAppId2);
  }
  async updateWalletState(sessionId, input) {
    const session = await this.getSession(sessionId);
  ...
  async getWalletTransactions(sessionId, walletAppId2) {
    ...
    if (!profile) return [];
    const transactions = await this.db.select().from(walletTransactions).where(eq(walletTransactions.walletAppId, walletAppId2)).orderBy(desc(walletTransactions.createdAt)).limit(50);
    const accountIds = new Set(
      (await this.db.select({ id: walletAccounts.id }).from(walletAccounts).where(eq(walletAccounts.walletProfileId, profile.id))).map((row) => row.id)
    );
  ...
  async buildWalletBootstrap(user, license, walletAppId2) {
    const wallet = { ...walletRegistry[walletAppId2], activated: true };
  ...
  async getLicense(licenseId) {
    const [license] = await this.db.select().from(licenses).where(eq(licenses.id, licenseId)).limit(1);
  ...
  async buildAffiliateDashboard(affiliate, baseUrl) {
    const [clicks, checkoutIntents, conversions, clickCount, intentCount, conversionCount, commissionRows] = await Promise.all([
      this.db.select().from(affiliateClicks).where(eq(affiliateClicks.affiliateId, affiliate.id)).orderBy(desc(affiliateClicks.createdAt)).limit(10),
      this.db.select().from(affiliateCheckoutIntents).where(eq(affiliateCheckoutIntents.affiliateId, affiliate.id)).orderBy(desc(affiliateCheckoutIntents.createdAt)).limit(10),
      this.db.select().from(affiliateConversions).where(eq(affiliateConversions.affiliateId, affiliate.id)).orderBy(desc(affiliateConversions.createdAt)).limit(10),
      this.db.select({ value: count() }).from(affiliateClicks).where(eq(affiliateClicks.affiliateId, affiliate.id)),
      this.db.select({ value: count() }).from(affiliateCheckoutIntents).where(eq(affiliateCheckoutIntents.affiliateId, affiliate.id)),
      this.db.select({ value: count() }).from(affiliateConversions).where(eq(affiliateConversions.affiliateId, affiliate.id)),
      this.db.select({ status: affiliateConversions.status, total: sum(affiliateConversions.commissionAmount) }).from(affiliateConversions).where(eq(affiliateConversions.affiliateId, affiliate.id)).groupBy(affiliateConversions.status)
    ]);
    ...
    const conversionSummaries = conversions.map(toAffiliateConversionSummary);
    const commission = /* @__PURE__ */ __name((status) => Number(commissionRows.find((entry) => entry.status === status)?.total || 0).toFixed(2), "commission");
    const totalCommission = commissionRows.filter((entry) => entry.status !== "rejected").reduce((total, entry) => total + Number(entry.total || 0), 0).toFixed(2);
    return {
      affiliate,
      referralUrl: `${baseUrl.replace(/\/+$/, "")}/r/${encodeURIComponent(affiliate.code)}`,
      stats: {
  ...
  async findAffiliateIntentForConversion(input) {
    if (input.checkoutIntentId) {
      const [intent] = await this.db.select().from(affiliateCheckoutIntents).where(eq(affiliateCheckoutIntents.id, input.checkoutIntentId)).limit(1);
      if (!intent) return void 0;
      if (input.affiliateCode && intent.affiliateCode !== normalizeAffiliateCode(input.affiliateCode)) return void 0;
  ...
  async createLicense(keyHash, userId, now) {
    const [license] = await this.db.insert(licenses).values({
      id: createId("lic"),
      keyHash,
      userId,
  ...
  async createSession(userId, licenseId, expiresAt) {
  ...
  async getOrCreateDemoKey() {
    const keyHash = await hashToken(DEMO_KEY_LABEL);
    const [existing] = await this.db.select().from(demoKeys).where(eq(demoKeys.keyHash, keyHash)).limit(1);
    if (existing) return existing;
    const [created] = await this.db.insert(demoKeys).values({
      id: createId("dky"),
      keyHash,
      label: DEMO_KEY_LABEL
    }).onConflictDoNothing({ target: demoKeys.keyHash }).returning();
    if (created) return created;
    return (await this.db.select().from(demoKeys).where(eq(demoKeys.keyHash, keyHash)).limit(1))[0];
  }
  async ensureWalletApps() {
    await this.db.insert(walletApps).values(Object.values(walletRegistry)).onConflictDoUpdate({
      target: walletApps.id,
      set: {
        name: walletApps.name,
        host: walletApps.host,
        enabled: walletApps.enabled
      }
    });
  }
  async getActivatedWallets(userId) {
    const profiles = await this.db.select({ walletAppId: walletProfiles.walletAppId }).from(walletProfiles).where(eq(walletProfiles.userId, userId));
    return profiles.map((profile) => profile.walletAppId);
  }
  async getOrCreateWalletProfile(userId, walletAppId2, displayName) {
    const [existing] = await this.db.select().from(walletProfiles).where(and(eq(walletProfiles.userId, userId), eq(walletProfiles.walletAppId, walletAppId2))).limit(1);
    const profile = existing ?? (await this.db.insert(walletProfiles).values({
      id: createId("wpf"),
      userId,
      walletAppId: walletAppId2,
      displayName,
  ...
  async getWalletProfileByUserAndApp(userId, walletAppId2) {
  ...
  async getWalletBalances(accounts) {
    ...
    const allBalances = await Promise.all(
      accounts.map(
        (account) => this.db.select().from(walletBalances).where(eq(walletBalances.accountId, account.id)).orderBy(asc(walletBalances.tokenSymbol))
      )
    );
  ...
  async getRecentWalletTransactions(accounts) {
    if (accounts.length === 0) return [];
    const accountIds = new Set(accounts.map((account) => account.id));
    const transactions = await this.db.select().from(walletTransactions).orderBy(desc(walletTransactions.createdAt)).limit(50);
    return transactions.filter((tx) => accountIds.has(tx.accountId)).map(toWalletTransaction);
  }
  async recordDevice(userId, deviceId) {
...
function buildAccessSummary(license, expiresAt) {
  return {
    kind: isDemoLicense(license) ? "demo" : "license",
    expiresAt
  };
}
__name(buildAccessSummary, "buildAccessSummary");
function isDemoLicense(license) {
  return license.plan === DEMO_LICENSE_PLAN;
}
__name(isDemoLicense, "isDemoLicense");
...
function isAffiliateSelfReferral(affiliateEmail, buyerEmail) {
  return Boolean(
    affiliateEmail && buyerEmail && affiliateEmail.trim().toLowerCase() === buyerEmail.trim().toLowerCase()
  );
}
__name(isAffiliateSelfReferral, "isAffiliateSelfReferral");
function normalizeTicketSubject(input) {
...
__name(normalizeTicketSubject, "normalizeTicketSubject");
function isSupportTicketType(value) {
  return value === "did_not_receive_key" || value === "bug";
}
__name(isSupportTicketType, "isSupportTicketType");
function isSupportTicketStatus(value) {
  return value === "open" || value === "in_progress" || value === "resolved" || value === "closed";
}
__name(isSupportTicketStatus, "isSupportTicketStatus");
...
function isAllowedAffiliateStatusTransition(current, next) {
  if (current === next) return true;
  if (current === "pending") return next === "approved" || next === "rejected";
  if (current === "approved") return next === "paid" || next === "rejected";
  return false;
}
__name(isAllowedAffiliateStatusTransition, "isAllowedAffiliateStatusTransition");
...
function toPaymentOrderSummary(order) {
  return {
    id: order.id,
    provider: order.provider,
    providerPaymentId: order.providerPaymentId ?? void 0,
    email: order.email,
    planId: order.planId,
    ...
    affiliateCode: order.affiliateCode ?? void 0,
    affiliateCheckoutIntentId: order.affiliateCheckoutIntentId ?? void 0,
    affiliateVisitorId: order.affiliateVisitorId ?? void 0,
    affiliateClickId: order.affiliateClickId ?? void 0,
    status: order.status,
    licenseId: order.licenseId ?? void 0,
...
__name(toPaymentOrderSummary, "toPaymentOrderSummary");
function toSupportTicketSummary(ticket) {
  return {
    id: ticket.id,
    type: ticket.type,
    ...
    orderId: ticket.orderId ?? void 0,
    providerPaymentId: ticket.providerPaymentId ?? void 0,
    transactionHash: ticket.transactionHash ?? void 0,
    paymentCurrency: ticket.paymentCurrency ?? void 0,
    amount: ticket.amount ?? void 0,
    adminNotes: ticket.adminNotes ?? void 0,
    createdAt: ticket.createdAt.toISOString(),
    updatedAt: ticket.updatedAt.toISOString()
  };
}
__name(toSupportTicketSummary, "toSupportTicketSummary");
...
function toAffiliateDashboardClick(click) {
  return {
    id: click.id,
    landingPath: click.landingPath,
    source: click.source,
...
__name(toAffiliateDashboardClick, "toAffiliateDashboardClick");
function toAffiliateDashboardIntent(intent) {
  return {
    id: intent.id,
    plan: intent.plan,
...
__name(toAffiliateDashboardIntent, "toAffiliateDashboardIntent");
function toAffiliateDashboardConversion(conversion) {
  return {
    id: conversion.id,
    plan: conversion.plan,
    amount: conversion.amount,
...
__name(toAffiliateDashboardConversion, "toAffiliateDashboardConversion");
...
function buildPayoutTotalsFromAggregates(affiliates2, rows) {
  return affiliates2.map((affiliate) => {
    const totalFor = /* @__PURE__ */ __name((status) => Number(rows.find((row) => row.affiliateId === affiliate.id && row.status === status)?.total || 0).toFixed(2), "totalFor");
    return {
      affiliateId: affiliate.id,
      affiliateCode: affiliate.code,
      pendingCommission: totalFor("pending"),
...
__name(buildPayoutTotalsFromAggregates, "buildPayoutTotalsFromAggregates");
...
function createDemoAddress(walletAppId2) {
  const prefix = walletAppId2 === "phantom" ? "Ph" : "Tw";
  return `${prefix}${crypto.randomUUID().replace(/-/g, "").slice(0, 30)}`;
}
__name(createDemoAddress, "createDemoAddress");
function getBalanceKey(accountId, tokenSymbol) {
  return `${accountId}:${tokenSymbol.toUpperCase()}`;
}
__name(getBalanceKey, "getBalanceKey");
function createBalanceRow(accountId, tokenSymbol, amount) {
  return {
    accountId,
...
function toWalletTransaction(tx) {
  return {
    ...
    amount: tx.amount,
    toTokenSymbol: tx.toTokenSymbol ?? void 0,
    toAmount: tx.toAmount ?? void 0,
    fromAddress: tx.fromAddress ?? void 0,
    toAddress: tx.toAddress ?? void 0,
    counterpartWalletAppId: tx.counterpartWalletAppId ?? void 0,
...
// src/payblis.ts
var PAYBLIS_CHECKOUT_BASE = "https://pay.payblis.com/api/payment_gateway.php";
var PAYBLIS_METHODS = "credit_cards,apple_pay,google_pay";
var PAYBLIS_WEBHOOK_MAX_BYTES = 64 * 1024;
...
__name(buildPayblisCheckoutUrl, "buildPayblisCheckoutUrl");
function parsePayblisWebhook(payload) {
...
__name(parsePayblisWebhook, "parsePayblisWebhook");
function payblisAmountMatches(expected, received) {
...
__name(payblisAmountMatches, "payblisAmountMatches");
function isFulfilledPayblisStatus(status) {
  return status === "SUCCESS";
}
__name(isFulfilledPayblisStatus, "isFulfilledPayblisStatus");
function payblisCurrencyMatches(expected, received) {
  return !received || received.toUpperCase() === expected.toUpperCase();
}
__name(payblisCurrencyMatches, "payblisCurrencyMatches");
async function verifyPayblisSignature(rawBody, payload, signature, secret) {
  ...
  if (embeddedSignature === void 0) {
    return timingSafeHexEqual(await hmacSha256Hex(secret, rawBody), normalizedSignature);
  }
  return false;
}
__name(verifyPayblisSignature, "verifyPayblisSignature");
function phpSerializeStringMap(map) {
  const encoder = new TextEncoder();
  const serializeString = /* @__PURE__ */ __name((value) => `s:${encoder.encode(value).length}:"${value}";`, "serializeString");
  const entries = Object.entries(map).map(([key, value]) => `${serializeString(key)}${serializeString(value)}`).join("");
  return `a:${Object.keys(map).length}:{${entries}}`;
}
__name(phpSerializeStringMap, "phpSerializeStringMap");
function base64Standard(value) {
  const bytes = new TextEncoder().encode(value);
  let binary = "";
  for (const byte of bytes) binary += String.fromCharCode(byte);
  return btoa(binary);
}
__name(base64Standard, "base64Standard");
...
__name(parseMoneyMinorUnits, "parseMoneyMinorUnits");
...
// src/referral-token.ts
var TOKEN_VERSION = 1;
async function createReferralToken(payload, secret) {
  assertSecret(secret);
...
__name(createReferralToken, "createReferralToken");
async function verifyReferralToken(token, secret) {
  ...
  try {
    ...
    const parsed = JSON.parse(new TextDecoder().decode(decodeBase64Url(encodedPayload)));
...
__name(verifyReferralToken, "verifyReferralToken");
...
__name(sign, "sign");
function isClickId(value) {
  return typeof value === "string" && /^afc_[a-f0-9]{18}$/.test(value);
}
__name(isClickId, "isClickId");
function isValidFutureDate(value) {
  if (typeof value !== "string") return false;
  const timestamp2 = Date.parse(value);
  return Number.isFinite(timestamp2) && timestamp2 > Date.now();
}
__name(isValidFutureDate, "isValidFutureDate");
...
var TOKENS = [
  ...
  { symbol: "BTC", price: 83e3, coingeckoId: "bitcoin" },
  ...
  { symbol: "MON", price: 1.5, coingeckoId: void 0 }
];
var CUSTOM_IMAGE_OVERRIDES = {
...
var EMAIL_FROM = "RPWallet <noreply@rpwallet.app>";
var app = new Hono2();
app.use("*", async (c, next) => {
...
app.get("/search", async (c) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return c.json({ error: "Failed to fetch token search results" }, response.status);
    }
    const data = await response.json();
...
app.get("/token-info", async (c) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return c.json({ error: "Failed to fetch token information" }, response.status);
    }
    const data = await response.json();
...
app.get("/affiliate/redirect/:code", async (c) => {
  ...
  const visitorId = requestedVisitorId && /^afv_[a-f0-9]{18}$/.test(requestedVisitorId) ? requestedVisitorId : createReferralVisitorId();
  ...
  const referralToken = result.accepted && result.click && result.attribution && trustedAttribution ? await issueReferralToken(c.env, result.click.id, result.attribution.expiresAt) : null;
...
app.post("/affiliate/click", async (c) => {
  ...
  return c.json({
    ...result,
    attribution: result.attribution && trustedAttribution ? { ...result.attribution, affiliateDisplayName: trustedAttribution.affiliateDisplayName } : result.attribution,
    referralToken: referralToken || void 0,
    claimCode: referralToken ? result.click?.id : void 0
  });
});
app.post("/affiliate/claim", async (c) => {
  const body = await c.req.json();
  let referralToken = normalizeBoundedString(body.referralToken, 1024);
  let attribution = referralToken ? await resolveReferralToken(getPlatformStore(c.env.DATABASE_URL), c.env, referralToken) : null;
  ...
  if (!attribution && claimCode && /^afc_[a-f0-9]{18}$/.test(claimCode)) {
    ...
    referralToken = attribution ? await issueReferralToken(c.env, attribution.clickId, attribution.expiresAt) || void 0 : void 0;
  }
...
app.post("/affiliate/recover", async (c) => {
  ...
  const referralToken = result.accepted && result.click && result.attribution && trusted ? await issueReferralToken(c.env, result.click.id, result.attribution.expiresAt) : null;
...
app.get("/admin/licenses/unused", async (c) => {
  if (!isAffiliateAdminRequest(c)) return c.json({ error: "Unauthorized" }, 401);
  const licenses2 = await getPlatformStore(c.env.DATABASE_URL).getAdminUnusedActiveLicenses();
  return c.json({ licenses: licenses2 });
});
app.post("/admin/licenses/send-reminder", async (c) => {
  ...
  if (snapshot.license.status !== "active" || new Date(snapshot.license.expiresAt) <= /* @__PURE__ */ new Date()) {
...
app.patch("/admin/tickets/:id", async (c) => {
  ...
  const ticket = await getPlatformStore(c.env.DATABASE_URL).updateAdminSupportTicket(id, {
    status: status && isSupportTicketStatus(status) ? status : void 0,
    adminNotes: body.adminNotes === void 0 ? void 0 : normalizePayloadString(body.adminNotes) || ""
  });
...
app.post("/payments/nowpayments/checkout", async (c) => {
  ...
  try {
    ...
    const email = normalizePayloadString(body.email)?.toLowerCase();
    const plan = planId ? PAYMENT_PLANS[planId] : void 0;
    ...
    const store = getPlatformStore(c.env.DATABASE_URL);
    const referralAttribution = body.referralToken ? await resolveReferralToken(store, c.env, body.referralToken) : null;
    const emailAttribution = referralAttribution ? null : await store.getAffiliateAttributionByBuyerEmail(email);
    const affiliateIntent = referralAttribution ? await store.createAffiliateCheckoutIntent({
    ...
    const trustedAttribution = affiliateIntent?.accepted && affiliateIntent.intent ? { attribution: referralAttribution, intent: affiliateIntent.intent, source: "current" } : emailAttribution ? { attribution: emailAttribution, intent: { id: emailAttribution.checkoutIntentId }, source: "email" } : null;
    const referralBonusDays2 = referralAttribution || emailAttribution ? getReferralBonusDays(plan.id) : 0;
    ...
    const order = await store.createPaymentOrder({
      ...
      priceAmount,
      priceCurrency: "USD",
      durationDays: plan.durationDays + referralBonusDays2,
    ...
    console.log("[nowpayments-checkout] Payment order created", {
      requestId,
      orderId: order.id,
      referralBonusDays: referralBonusDays2,
...
async function handleNowPaymentsWebhook(c) {
...
__name(handleNowPaymentsWebhook, "handleNowPaymentsWebhook");
async function fulfillPaidOrder(c, order, options) {
  ...
  const expiresAt = new Date(Date.now() + order.durationDays * 24 * 60 * 60 * 1e3);
...
__name(fulfillPaidOrder, "fulfillPaidOrder");
async function ensureAffiliateConversion(c, order, options) {
...
__name(ensureAffiliateConversion, "ensureAffiliateConversion");
function isFulfilledNowPaymentsStatus(status) {
  return status === "finished";
}
__name(isFulfilledNowPaymentsStatus, "isFulfilledNowPaymentsStatus");
...
async function handlePayblisWebhook(c) {
...
__name(handlePayblisWebhook, "handlePayblisWebhook");
...
async function handleSellAuthWebhook(c) {
  ...
  const emailAttribution = buyerEmail ? await store.getAffiliateAttributionByBuyerEmail(buyerEmail) : null;
  const referralBonusDays2 = emailAttribution ? getReferralBonusDays(plan.id) : 0;
  const now = Date.now();
  const expiresAt = new Date(now + (plan.durationDays + referralBonusDays2) * 24 * 60 * 60 * 1e3);
  const license = await store.createPurchasedLicense({
    licenseKey,
    email: buyerEmail,
    plan: plan.label,
  ...
  c.executionCtx.waitUntil(
    ...
    }).then((result) => notifyReferralConversion(c.env, result)).catch((error) => {
...
app.get("/prices", async (c) => {
  try {
    ...
    if (!response.ok) {
      return c.json({ error: "Failed to fetch from CoinGecko" }, response.status);
    }
    const data = await response.json();
    const prices = {};
    const idToSymbols = {};
    ...
    for (const coin of data) {
      ...
      for (const symbol of symbols) {
        prices[symbol] = {
          image: CUSTOM_IMAGE_OVERRIDES[symbol] || (coin.image ? `https://api.phantom.app/image-proxy/?image=${encodeURIComponent(coin.image)}&anim=false&fit=cover&width=128&height=128` : void 0),
...
app.get("/chart", async (c) => {
  try {
      ...
      try {
        ...
        if (dsResponse.ok) {
          ...
          if (dsData.pairs && dsData.pairs.length > 0) {
            ...
            const timeframeMs = timeframe === "1H" ? 60 * 60 * 1e3 : timeframe === "1D" ? 24 * 60 * 60 * 1e3 : 7 * 24 * 60 * 60 * 1e3;
    ...
    if (!response.ok) {
      ...
      return c.json({ error: "Failed to fetch chart data" }, response.status);
    }
    const data = await response.json();
    let prices = data.prices || [];
    if (timeframe === "1H" && prices.length > 0) {
      const oneHourAgo = Date.now() - 60 * 60 * 1e3;
      prices = prices.filter((point2) => point2[0] >= oneHourAgo);
    }
    c.header("Cache-Control", "public, s-maxage=60, stale-while-revalidate=300");
    return c.json(prices);
  } catch (error) {
...
app.get("/token-details", async (c) => {
  try {
    ...
    if (!response.ok) {
      return c.json({ error: "Failed to fetch token details" }, response.status);
    }
    const data = await response.json();
...
app.delete("/wallet-accounts/:accountId", async (c) => {
  ...
  const response = await getPlatformStore(c.env.DATABASE_URL).deleteWalletAccount(sessionId, walletAppId2, c.req.param("accountId"));
  if (!response) {
...
function applyWalletAvailabilityToHubSession(env, response) {
  const wallets = response.access?.kind === "demo" ? response.wallets.filter((wallet) => wallet.id === "phantom") : response.wallets;
  return {
    ...response,
...
function getSessionCookieSecurity(c) {
  const isHttps = new URL(c.req.url).protocol === "https:";
  return isHttps ? { secure: true, sameSite: "None" } : { secure: false, sameSite: "Lax" };
}
__name(getSessionCookieSecurity, "getSessionCookieSecurity");
function buildIdMap() {
  const map = {};
  for (const token of TOKENS) {
...
function buildLaunchUrl(env, walletAppId2, token, returnTo, deviceId) {
...
async function verifySellAuthSignature(rawBody, signature, secret) {
  const normalizedSignature = normalizeSignature2(signature);
  if (!normalizedSignature) return false;
  const rawComputed = await hmacSha256Hex2(secret, rawBody);
  if (timingSafeHexEqual2(rawComputed, normalizedSignature)) return true;
  const canonicalBody = canonicalizeJsonString(rawBody);
  if (!canonicalBody || canonicalBody === rawBody) return false;
  const canonicalComputed = await hmacSha256Hex2(secret, canonicalBody);
  return timingSafeHexEqual2(canonicalComputed, normalizedSignature);
}
...
async function buildSellAuthSignatureDiagnostics(rawBody, signature, secret) {
  ...
  const rawComputed = await hmacSha256Hex2(secret, rawBody);
  const canonicalBody = canonicalizeJsonString(rawBody);
  const canonicalComputed = canonicalBody && canonicalBody !== rawBody ? await hmacSha256Hex2(secret, canonicalBody) : null;
  return {
...
async function hmacSha256Hex2(secret, message) {
  const key = await crypto.subtle.importKey(
    "raw",
    new TextEncoder().encode(secret),
    { name: "HMAC", hash: "SHA-256" },
...
async function verifyNowPaymentsSignature(payload, signature, secret) {
  const canonicalPayload = JSON.stringify(sortObjectKeys(payload));
  const computed = await hmacSha512Hex(secret, canonicalPayload);
  return timingSafeHexEqual2(computed, normalizeSignature2(signature));
}
__name(verifyNowPaymentsSignature, "verifyNowPaymentsSignature");
...
function timingSafeHexEqual2(leftHex, rightHex) {
  const left = hexToBytes3(leftHex);
  const right = hexToBytes3(rightHex);
  if (!left || !right || left.length !== right.length) return false;
  let diff = 0;
...
__name(timingSafeHexEqual2, "timingSafeHexEqual");
...
async function generateLicenseKeyForOrder(orderId, secret) {
  const digest = await hmacSha256Hex2(secret, `license:${orderId}`);
  const alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  const letters = Array.from(hexToBytes3(digest).slice(0, 20), (byte) => alphabet[byte % alphabet.length]);
...
function extractSellAuthAmount(payload) {
  const value = payload.paid ?? payload.total ?? payload.total_usd ?? payload.item?.total_price ?? payload.price ?? payload.data?.paid ?? payload.data?.total ?? payload.data?.total_usd ?? payload.data?.item?.total_price ?? payload.data?.price ?? payload.amount ?? payload.data?.amount;
  if (value === void 0 || value === null) return void 0;
...
async function sendLicenseReminderEmail(env, params) {
  if (!env.RESEND_API_KEY) {
...
__name(sendLicenseReminderEmail, "sendLicenseReminderEmail");
async function sendPaymentFailedEmail(env, params) {
  if (!env.RESEND_API_KEY) {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      subject: params.reason === "expired" ? "Your RPWallet payment expired" : "Your RPWallet payment didn't go through",
      to: params.to
    })
  });
  if (!response.ok) {
...
async function sendAbandonedCheckoutEmail(env, params) {
  if (!env.RESEND_API_KEY) {
...
__name(sendAbandonedCheckoutEmail, "sendAbandonedCheckoutEmail");
async function notifyReferralConversion(env, result) {
...
__name(notifyReferralConversion, "notifyReferralConversion");
async function sendReferralConversionEmail(env, params) {
  if (!env.RESEND_API_KEY) {
...
__name(sendReferralConversionEmail, "sendReferralConversionEmail");
var EMAIL_BRAND = {
...
function renderSystemTag(label) {
  const b2 = EMAIL_BRAND;
  return `<p style="margin:0 0 10px;color:${b2.purple};font-family:${b2.monoStack};font-size:11px;font-weight:700;letter-spacing:0.22em;text-transform:uppercase;text-align:center;">[ ${escapeHtml(label)} ]</p>`;
}
__name(renderSystemTag, "renderSystemTag");
var EMAIL_ICON_ASPECT = {
...
function renderIcon(file, height, extraStyle = "") {
...
function renderGlassIconTile(file, tile, iconHeight) {
  const b2 = EMAIL_BRAND;
  return `<table role="presentation" width="${tile}" height="${tile}" cellpadding="0" cellspacing="0" style="width:${tile}px;height:${tile}px;border-radius:8px;background:${b2.cardBg};background-image:${b2.glassFill};border:1px solid ${b2.glassBorder};box-shadow:${b2.glassHighlight};">
  <tr><td align="center" valign="middle" style="text-align:center;">${renderIcon(file, iconHeight, "margin:0 auto;")}</td></tr>
</table>`;
}
__name(renderGlassIconTile, "renderGlassIconTile");
function buildBrandEmailShell(opts) {
  const b2 = EMAIL_BRAND;
  return `
<!doctype html>
<html lang="en">
  ...
  <body style="margin:0;padding:0;background:${b2.pageBg};color:#ffffff;font-family:${b2.fontStack};-webkit-font-smoothing:antialiased;">
    <div style="display:none;max-height:0;overflow:hidden;opacity:0;color:${b2.pageBg};font-size:1px;line-height:1px;">
      ${escapeHtml(opts.preheader)}
    </div>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${b2.pageBg};">
      <tr>
        <td align="center" style="padding:32px 16px;">
          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:100%;max-width:600px;">
            <tr>
              <td align="center" style="padding:8px 0 14px;">
                <img src="${b2.assetBase}/logo-ghost.png" width="36" height="34" alt="" style="display:inline-block;vertical-align:middle;border:0;" />
                <span style="display:inline-block;vertical-align:middle;margin-left:10px;font-size:20px;font-weight:800;letter-spacing:0.01em;color:#ffffff;">RP<span style="color:${b2.purple};">Wallet</span></span>
              </td>
            </tr>
            <tr>
              <td align="center" style="padding:0 0 16px;">
                <span style="display:inline-block;padding:4px 14px;border:1px solid ${b2.purpleBorder};border-radius:3px;background:${b2.keyBg};color:${b2.purple};font-family:${b2.monoStack};font-size:10px;font-weight:700;letter-spacing:0.3em;text-transform:uppercase;box-shadow:0 0 14px rgba(139,92,246,0.35);">System&nbsp;Message</span>
              </td>
            </tr>
            <tr>
              <td style="padding:0;">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-radius:12px;background:${b2.purpleBorder};background-image:linear-gradient(180deg,rgba(192,132,252,0.6),rgba(139,92,246,0.3) 45%,rgba(192,132,252,0.5));box-shadow:0 24px 70px rgba(0,0,0,0.55),0 0 44px rgba(124,58,237,0.22);">
                  <tr><td style="padding:1px;">
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-radius:11px;background:${b2.cardBg};background-image:${b2.glassFill};box-shadow:${b2.glassHighlight};overflow:hidden;">
            ...
            <tr>
              <td align="center" style="padding:26px 20px 8px;">
                <p style="margin:0 0 6px;color:rgba(255,255,255,0.5);font-size:12px;line-height:1.5;">
                  Need help? <a href="${b2.telegramUrl}" style="color:${b2.purple};text-decoration:none;">Message us on Telegram</a>
                </p>
                <p style="margin:0;color:rgba(255,255,255,0.32);font-size:11px;line-height:1.5;">
                  &copy; RPWallet &middot; <a href="${b2.siteUrl}" style="color:rgba(255,255,255,0.45);text-decoration:none;">rpwallet.app</a>
                </p>
              </td>
            </tr>
          </table>
...
__name(buildBrandEmailShell, "buildBrandEmailShell");
function renderEmailCta(label, href) {
  const b2 = EMAIL_BRAND;
  return `
<table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 auto;">
  <tr><td align="center" style="border-radius:4px;border:1px solid rgba(196,181,253,0.6);background:${b2.accent};background-image:linear-gradient(180deg,#9b7bff 0%,#7c3aed 45%,#5b21b6 100%);box-shadow:inset 0 1px 0 rgba(255,255,255,0.35),0 0 24px rgba(139,92,246,0.45),0 12px 30px rgba(88,28,180,0.35);">
...
__name(renderEmailCta, "renderEmailCta");
function renderKeyPanel(licenseKey) {
  const b2 = EMAIL_BRAND;
  return `
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;">
  <tr><td style="padding:24px 18px;border-radius:8px;background:${b2.keyBg};background-image:${b2.glassPurpleFill};border:1px solid ${b2.purpleBorder};box-shadow:${b2.glassPurpleGlow};text-align:center;">
    <p style="margin:0 0 10px;color:${b2.purple};font-family:${b2.monoStack};font-size:11px;font-weight:700;letter-spacing:0.22em;text-transform:uppercase;">[ License Key ]</p>
    <div style="font-family:${b2.monoStack};font-size:22px;font-weight:800;letter-spacing:0.1em;color:#ffffff;word-break:break-all;">
      ${escapeHtml(licenseKey)}
    </div>
  </td></tr>
</table>`;
}
__name(renderKeyPanel, "renderKeyPanel");
function renderBadgePair(label1, value1, label2, value2) {
  const b2 = EMAIL_BRAND;
  const cell = /* @__PURE__ */ __name((label, value) => `
    <td width="50%" style="padding:0 4px;">
      <div style="padding:13px 15px;border-radius:6px;background:${b2.cardBg};background-image:${b2.glassFill};border:1px solid ${b2.glassBorder};box-shadow:${b2.glassHighlight};">
        <p style="margin:0 0 3px;color:${b2.purple};font-family:${b2.monoStack};font-size:10px;letter-spacing:0.18em;text-transform:uppercase;">${label}</p>
...
__name(renderBadgePair, "renderBadgePair");
function renderMetaBadges(planLabel, expirationDate) {
  return renderBadgePair("Plan", planLabel, "Expires", expirationDate);
}
__name(renderMetaBadges, "renderMetaBadges");
function renderActivationSteps() {
  const b2 = EMAIL_BRAND;
  const step = /* @__PURE__ */ __name((icon, iconH, n, title, copy) => `
    <tr>
  ...
  return `
<p style="margin:0 0 14px;color:${b2.purple};font-family:${b2.monoStack};font-size:11px;font-weight:700;letter-spacing:0.22em;text-transform:uppercase;">[ Activation quest ]</p>
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 26px;">
  ${step("iphone-icon.png", 30, "1", "Open the site", `Go to <a href="${b2.siteUrl}" style="color:${b2.purple};text-decoration:none;">rpwallet.app</a> in Safari (iOS) or Chrome (Android) \u2014 not an in-app browser.`)}
...
__name(renderActivationSteps, "renderActivationSteps");
function buildPurchaseEmailHtml(params) {
  const b2 = EMAIL_BRAND;
  const body = `
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 18px;">
  <tr><td align="center">
    <table role="presentation" width="92" height="92" cellpadding="0" cellspacing="0" style="width:92px;height:92px;margin:0 auto;border-radius:10px;background:${b2.cardBg};background-image:${b2.glassPurpleFill};border:1px solid ${b2.purpleBorder};box-shadow:${b2.glassPurpleGlow};">
...
${renderEmailCta("Activate now", b2.siteUrl)}`;
...
__name(buildLicenseReminderEmailHtml, "buildLicenseReminderEmailHtml");
function buildPaymentFailedEmailHtml(params) {
  const b2 = EMAIL_BRAND;
  const telegramUrl = b2.telegramUrl;
  const expired = params.reason === "expired";
  const headline = expired ? "Your payment window expired" : "Your payment didn't go through";
  const lead = expired ? `Your <strong style="color:#fff;">${escapeHtml(params.planLabel)}</strong> invoice expired before a payment was confirmed. If you didn't finish paying, nothing was sent \u2014 just grab your access with a fresh checkout below.` : `We couldn't confirm your <strong style="color:#fff;">${escapeHtml(params.planLabel)}</strong> payment. If you didn't complete it, no crypto left your wallet \u2014 start a fresh checkout below whenever you're ready.`;
  const body = `
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 18px;">
  <tr><td align="center">
    <table role="presentation" width="92" height="92" cellpadding="0" cellspacing="0" style="width:92px;height:92px;margin:0 auto;border-radius:10px;background:${b2.cardBg};background-image:${b2.glassPurpleFill};border:1px solid ${b2.purpleBorder};box-shadow:${b2.glassPurpleGlow};">
...
${renderEmailCta("Try checkout again", `${b2.siteUrl}/buy`)}
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:24px 0 0;">
  <tr><td style="padding:16px 18px;border-radius:8px;background:${b2.cardBg};background-image:${b2.glassFill};border:1px solid ${b2.glassBorder};box-shadow:${b2.glassHighlight};text-align:center;">
    ...
    <a href="${telegramUrl}" style="display:inline-block;padding:10px 20px;border-radius:4px;background:rgba(139,92,246,0.16);border:1px solid ${b2.purpleBorder};color:${b2.purple};font-size:13px;font-weight:700;letter-spacing:0.04em;text-decoration:none;">Contact us on Telegram</a>
  </td></tr>
</table>`;
  return buildBrandEmailShell({
    preheader: expired ? "Your payment expired \u2014 nothing was charged. Grab your access, or reach us on Telegram if you already paid." : "We couldn't confirm your payment. Start a new checkout, or reach us on Telegram if you already paid.",
    bodyHtml: body
  });
}
__name(buildPaymentFailedEmailHtml, "buildPaymentFailedEmailHtml");
...
${renderEmailCta("Finish your purchase", `${b2.siteUrl}/buy`)}
<p style="margin:22px 0 0;color:rgba(255,255,255,0.42);font-size:12px;line-height:1.5;text-align:center;">
  Ran into trouble paying? <a href="${b2.telegramUrl}" style="color:${b2.purple};text-decoration:none;">Message us on Telegram</a> and we'll help you finish.
</p>`;
  return buildBrandEmailShell({
...
__name(buildAbandonedCheckoutEmailHtml, "buildAbandonedCheckoutEmailHtml");
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;">
  <tr><td style="padding:24px 18px;border-radius:8px;background:${b2.keyBg};background-image:${b2.glassPurpleFill};border:1px solid ${b2.purpleBorder};box-shadow:${b2.glassPurpleGlow};text-align:center;">
    <p style="margin:0 0 8px;color:${b2.purple};font-family:${b2.monoStack};font-size:11px;font-weight:700;letter-spacing:0.22em;text-transform:uppercase;">[ Your commission ]</p>
...
__name(buildReferralConversionEmailHtml, "buildReferralConversionEmailHtml");
...
function getReferralSigningSecret(env) {
  return env.REFERRAL_SIGNING_SECRET || "";
}
__name(getReferralSigningSecret, "getReferralSigningSecret");
async function issueReferralToken(env, clickId, expiresAt) {
...
__name(issueReferralToken, "issueReferralToken");
async function resolveReferralToken(store, env, token) {
  const secret = getReferralSigningSecret(env);
...
__name(isLinkPreviewBot, "isLinkPreviewBot");
function inferReferralSource(referrer, userAgent) {
...
__name(inferReferralSource, "inferReferralSource");
...
__name(getDemoConfig, "getDemoConfig");
...
async function runAbandonedCheckoutSweep(env) {
...
__name(runAbandonedCheckoutSweep, "runAbandonedCheckoutSweep");
var src_default = {
  fetch: app.fetch,
...
// .wrangler/tmp/bundle-uQsGOF/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-uQsGOF/middleware-loader.entry.ts
